...
modify_windows() {
    ...
    if is_need_change_rdp_port; then
        create_win_change_rdp_port_script $os_dir/SetupComplete.bat "$rdp_port" "$pas_rdp"
        bats="$bats SetupComplete.bat"
    fi
...
create_win_change_rdp_port_script() {
    ...
    echo "set PasRdp=$pas_rdp" >$target
    wget $confhome/SetupComplete.bat -O- >>$target
    unix2dos $target
}
